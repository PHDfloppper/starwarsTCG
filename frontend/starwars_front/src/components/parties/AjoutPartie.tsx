    ...
    */
    ...
    const handleAjout = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await axios.post('http://localhost:3000/api/parties/ajouter', nouvellePartie);
            if (resultat == "victoire") {
                await axios.put(`http://localhost:3000/api/decks/victoire/${deckUtilise?._id}`)
            }
            setMessage('partie ajouté avec succès !');
        } catch (err) {
            console.error(err);
